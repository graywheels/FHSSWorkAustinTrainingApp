...
export class UserTableComponent {
    ...
    interaction: {
      ...
      onClick: (data) => {
        ...
        editRef.afterClosed().subscribe((action) => {
          if (
            action === 'delete' &&
            this.table().dataResource.value()?.data.length === 1
          ) {
  ...
  forceBefore = (d: Date | null): boolean =>
    !d || !this.createdBefore || d < this.createdBefore;
  createdBefore: Date | undefined;
  forceAfter = (d: Date | null): boolean =>
    !d || !this.createdAfter || d > this.createdAfter;
